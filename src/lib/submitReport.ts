...
export function buildSubmitPayload(state: ReportFormState, turnstileToken: string): SubmitReportPayload {
  const report: NewReport = {
    ...
    date_first_offered: state.stillWaiting ? null : state.dateFirstOffered || null,
    date_seen: state.stillWaiting ? null : state.dateSeen || null,
    still_waiting: state.stillWaiting,
